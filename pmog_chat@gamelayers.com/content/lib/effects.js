...
Element.getOpacity = function(element){  
  var opacity;  
  if ((opacity = Element.getStyle(element, "opacity")) != null)  
    return parseFloat(opacity);  
  if ((opacity = (Element.getStyle(element, "filter") || '').match(/alpha\(opacity=(.*)\)/)) != null)  
...
Element.setOpacity = function(element, value){  
  element= $(element);  
  // Had an error here, just trying to reproduce it before including this fix.  SEC
  //if (!element) return;
...
var Effect = {
  tagifyText: function(element) {
    ...
    element = $(element);
    $PA(element.childNodes).each( function(child) {
      if(child.nodeType==3) {
  ...
  multiple: function(element, effect) {
    ...
    $PA(elements).each( function(element, index) {
...
Effect.Queue = {
  ...
  add: function(effect) {
    ...
    if(!this.interval) {
/*      dump("this.loop = " + this.loop);    
      dump("this.loop.bind = " + this.loop.bind(this));*/
      //var f = this.loop;
      var f = eval(this.loop.toString()); // This is weird, and I'm not proud of it, but I'm trying to make it work.
      
      this.interval = setInterval(f.bind(this), 40); // this.loop.bind is not a function.

      //this.interval = setInterval(this.loop.bind(this), 40); // this.loop.bind is not a function.
    }
  },
